# 公司/律所画像

*由所有 Claude for Legal 插件共享。你首次设置的插件写入此文件；其余插件读取。直接编辑或重新运行任意插件的冷启动面试来更新。*

**实务环境：** [个人执业/小型律所 | 中型/大型律所 | 企业内部法务 | 政府/法律援助/法律诊所]
**名称：** [公司或律所名称]
**行业：** [公司主营业务 / 律所主要业务领域]
**我们提供什么：** [产品、服务、面向谁——或填写"不适用，律所"]
**规模：** [员工数 / 律师数 / 相关人数]

## 地理和监管版图

**经营覆盖省份/地区：** [如：浙江（宁波、杭州）、上海、北京]
**主要管辖地：** [大部分业务发生的地区]
**所受监管机构：** [市场监管总局、税务局、网信办、金融监管局等——仅列适用的]
**正在进行的监管事项：** [或填"无"]

## 风险姿态

**整体风险偏好：** [保守 / 中等 / 积极]
**让我们睡不着的：** [会造成非常糟糕后果的事项]
**管理层最常问的问题：** [或尚未知晓]

## 关键人员

**法务负责人 / 主任：** [姓名]
**升级链条：** [姓名 → 姓名 → 姓名，或"按插件分别设置"]

## 本地知识库

*若你维护本地法律知识库（法规汇编、理解与适用、裁判文书等），在此填写其根目录的绝对路径。各插件的"知识库检索路由"会读取 `[知识库根目录]/.claude/rules/knowledge-routing.md` 作为检索配置。*

**知识库根目录：** [PLACEHOLDER — 例如 `~/Documents/知识库`；留空则跳过本地检索]

*未配置时，插件不读取本地路径，直接退化为 yuan dian MCP / 联网检索 + 模型知识，并打 `[模型知识 — 需验证]` 标签。绝不去读一个不存在的硬编码路径。*

---

*各插件的实践画像（审查指引、审查框架、内部风格、事项工作空间）与本文件并列存放在各插件文件夹中。本文件存放的是无论使用哪个插件都通用的事实。*
